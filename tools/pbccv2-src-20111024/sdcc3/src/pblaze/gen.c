...
/*-----------------------------------------------------------------*/
/* genSignCmp - compare left < right                               */
/*-----------------------------------------------------------------*/
static void genSignCmp(iCode * ic, operand * left, operand * right, operand * result, iCode * ifx)
{
    int size;
    unsigned long lit = 0L;
    value *aop_lit = NULL;
    symbol *lblo, *lble, *lblns;
    regs *reg;
    //sym_link *rtype = operandType(right);

    D(pblaze_emitcode(";", "genSignedCompare"));
    ...
    if (ifx) {
        ...
        if (isOperandLiteral(right)) {
            ...
            if (IC_TRUE(ifx)) {
                pblaze_emitcode("TEST", "%s, %s", aopGetRegName(ic, left, size-1), dialectNum(128));
                if(lit < NEG_VAL) {
                    pblaze_emitcode("JUMP", "C, _L%05d", LBL_KEY(IC_TRUE(ifx)));
                }
                else {
                     pblaze_emitcode("JUMP", "NC, _LC%05d", LBL_KEY(lble) );   
                }
            ...
            /* false label will be generated */
                ...
                if(lit < NEG_VAL) {
                    pblaze_emitcode("JUMP", "C, _LC%05d", LBL_KEY(lble) );
                     ...
                     pblaze_emitcode("JUMP", "NC, _L%05d", LBL_KEY(IC_FALSE(ifx)) );   
                }
        ...
        /* left operand is a literal value */
            ...
            if (IC_TRUE(ifx)) { 
                ...
                pblaze_emitcode("TEST", "%s, %s", aopGetRegName(ic, right, size-1), dialectNum(128));
                if(lit < NEG_VAL) {
                    pblaze_emitcode("JUMP", "C, _LC%05d", LBL_KEY(lble) );
                }
                else {
                     pblaze_emitcode("JUMP", "NC, _L%05d", LBL_KEY(IC_TRUE(ifx)) );   
                }
            ...
            /* false label will be generated */
                ...
                pblaze_emitcode("TEST", "%s, %s", aopGetRegName(ic, right, size-1), dialectNum(128));
                if(lit  < NEG_VAL) {
                    pblaze_emitcode("JUMP", "C, _L%05d", LBL_KEY(IC_FALSE(ifx)) );
                }
                else {
                     pblaze_emitcode("JUMP", "NC, _LC%05d", LBL_KEY(lble) );   
                }
        ...
        /* both are in the registers */
            ...
            if (IC_TRUE(ifx)) {
                ...
                pblaze_emitcode("LOAD", "%s, %s", reg->name, aopGetRegName(ic, left, size-1));
                pblaze_emitcode("XOR", "%s, %s", reg->name, aopGetRegName(ic, right, size-1));
                pblaze_emitcode("AND", "%s, %s", reg->name, dialectNum(128));
                unlockReg(reg);
                
                pblaze_emitcode("JUMP", "Z, _LC%05d", LBL_KEY(lblo));
    ...
    /* next instruction is not IFX */
        ...
        if (size == 1) {
            ...
            if (isOperandLiteral(right)) {
                ...
                pblaze_emitcode("TEST", "%s, %s", aopGetRegName(ic, left, 0), dialectNum(128));
                
                if(lit  < NEG_VAL) {
                    pblaze_emitcode("JUMP", "NC, _LC%05d", LBL_KEY(lblo));
                    aopPutVal(ic, result, dialectNum(1), 0);
                    aopUpdateOpInMem(ic, result, 0);
                    pblaze_emitcode("JUMP", "_LC%05d", LBL_KEY(lble));
                }
            ...
            /* left side is a literal value */
                ...
                pblaze_emitcode("TEST", "%s, %s", aopGetRegName(ic, right, 0), dialectNum(128));
                ...
                pblaze_emitcode("LOAD", "%s, %s", reg->name, aopGetRegName(ic, left, 0));
                pblaze_emitcode("XOR", "%s, %s", reg->name, aopGetRegName(ic, right, 0));
                pblaze_emitcode("AND", "%s, %s", reg->name, dialectNum(128));
                unlockReg(reg);
            ...
            if (isOperandLiteral(right)) {
                ...
                if(lit  < NEG_VAL) {
                    pblaze_emitcode("JUMP", "NC, _LC%05d", LBL_KEY(lblns));
                    aopPutVal(ic, result, dialectNum(1), 0);
                    aopUpdateOpInMem(ic, result, 0);
                    pblaze_emitcode("JUMP", "_LC%05d", LBL_KEY(lble));
                }
                else {
                    pblaze_emitcode("JUMP", "C, _LC%05d", LBL_KEY(lblns));
                    pblaze_emitcode("JUMP", "_LC%05d", LBL_KEY(lble));
                }
                              
                pblaze_emitLabelC(lblns);
                while (size--) {
                    pblaze_emitcodeCompare(aopGetRegName(ic, left, size), dialectNum(valueOffset(lit, size)));
                    pblaze_emitcode("JUMP", "C, _LC%05d", LBL_KEY(lblo));
                }
            ...
            /* left side is a literal value */
                ...
                pblaze_emitcode("JUMP", "Z, _LC%05d", LBL_KEY(lblns));
...
static void genCmp(iCode * ic, operand * left, operand * right, operand * result, iCode * ifx)
{
    ...
    if( !(SPEC_USIGN(ltype) && SPEC_USIGN(rtype)) ) {
        genSignCmp(ic, left, right, result, ifx);
        return;
    }
...
static void genCast(iCode * ic)
{
    ...
    /* the size of destination is greater than the size of the source */
        ...
        if(SPEC_USIGN(rtype)) {
            if(opt) {
                for (i = sizeRight; i < sizeRes; i++) {
                    pblaze_emitcode("LOAD", "%s, %s", aopGetRegName(ic, result, i), dialectNum(0));
                    aopUpdateOpInMem(ic, result, i);
                }
            }
        }
